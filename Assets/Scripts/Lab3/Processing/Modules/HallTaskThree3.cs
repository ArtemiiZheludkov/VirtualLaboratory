...
{
    public class HallTaskThree3 : ProcessModule
    {
        ...
        protected override void ShowProcessResult()
        {
            ...
            float[] teta = new float[data.B.Length];
            for (int i = 0; i < teta.Length; i++)
                teta[i] = Mathf.Atan(w[i] * t[i]);

            string text1 = "R<sub>H</sub>=" +R.ToString("#0.00") +" (Om); " 
                           +"σ<sub>H</sub>=" +sigma.ToString("#0.000") +" (1/Om*cm); ";
            
            string text2 = "θ=arctg(ω*τ) (рад) => "
                           +$"θ<sub>{data.B[0]}</sub>=" +teta[0].ToString("#0.00") +"; ";
            
            string text3 = $"θ<sub>{data.B[1]}</sub>=" +teta[1].ToString("#0.00") + "; "
                           +$"θ<sub>{data.B[2]}</sub>=" +teta[2].ToString("#0.00") +"; ";

            string text4 = $"θ<sub>{data.B[3]}</sub>=" +teta[3].ToString("#0.00") + "; "
                           +$"θ<sub>{data.B[4]}</sub>=" +teta[4].ToString("#0.00") + ";";
